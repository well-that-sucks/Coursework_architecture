...
model = Model(open(FilePaths.fnCharList).read(), DecoderType.BestPath, mustRestore=False)
...
#
    imgs_misc = load_different_image()
    imgs = [img] + imgs_misc
...
def run(fnImgs = []):
    ...
    """ Recognize text in image provided by file path """
    recognized_text = []
    for img_name in fnImgs:
        if img_name != '':
            recognized_text.append(infer(model, img_name))
