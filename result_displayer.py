...
class ResultsDisplayer:
    ...
    def __on_key_down(self, event):
        for text_widget in self.text_widgets:  
            ...
            text_widget.insert(END, text[:495])

    def __on_button_click(self, filename, text_widget):
        result_filename = "results/" + filename + "_result"
        text = str(text_widget.get("1.0", END)).strip()
        for char in text:
            if ord(char) < 32 or ord(char) > 126:
                messagebox.showwarning('Invalid input', 'Your input contains invalid character(s)! Try again.')
                return False
